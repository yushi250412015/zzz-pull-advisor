...
- **【决策质量修复（可用性验证发现）】**：①机会成本改按「期望消耗抽数」计费（`expectedSpentPulls`：拿到即停，小额低于预算、大额收敛到期望 93.4 抽；修掉全额预算双重高估成本导致结论永远偏保守的缺陷）+2 单测；②推荐卡对**已拥有角色**显示「已拥有（影画收益暂未建模）」不再给误导性推荐；③实测校准认知：50/50 角色池期望拿 UP 实为 **93.4 抽**（非此前文档的 62.5——为配置演进遗留的旧标注，已如实记录）
- 测试 141 项（+2 expectedSpentPulls）
- **【meta 全量校准完成 31/31】**：维琳娜（最后一个 null）双源合成定值 80（卡特亚官方搭档 82 vs 毕加「一般」75 观测折中 → 后验 ≈78.9）；星见雅旧先验 92→88（冲突折中）；艾莲/朱鸢 历史先验注记；7 个缺 note 角色补来源；新增不变量单测 `characters-data.test.js`（meta 非 null + 值域 + note 依据）
- 测试 139 项（+2 不变量）
- **【老角色信息更新（既有视频源）】**：分页拉取卡特亚/毕加丶完整视频清单，定位并消化老角色视频 7+ 条；observations +2（妮可 85「强度登顶」毕加标题级、莱卡恩 74「常驻S优先级第一」评论区共识）；mechanics **新增 14 个老角色条目**（艾莲/朱鸢/莱卡恩/格莉丝/11号/丽娜/猫又/安比/比利/可琳/派派/露西/赛斯/安东/本，全部带 BV 出处）；characters.js 老角色 note 补来源与「旧占位先验」标注；开服期与现行数据按日期区分
- **【v2.1 全部事项】**：①潜能觉醒共识落库（猫又 75「最强物理强攻」/丽娜 62「强攻队万能插件」两条 bili-comments 观测 + 蕾米埃尔「利好异放」facts + 卢西娅佐证）；②希格莉德倍率：B站搜索无文本倍率表 → 维持 null 不臆造（标题共识「数值怪」佐证 meta）；③`scripts/sensitivity-equipment.mjs` 参数敏感性分析（蕾米专武 skip→consider 翻转点 0.45-0.6、希格莉德专武恒 skip=未拥有诚实行为、艾瑞儿恒 pull）；④**UI jsdom 冒烟测试 +8 项**（8 面板真实渲染断言，jsdom devDependency）；⑤SOP 演练（3.2 未进窗口）
- 测试 137 项（+8 UI 冒烟）
...
- 测试 79 项（+14 公告数据源）
- **全量四池账号解析**（2026-08-15，真实账号 uid 已脱敏）：常驻 109 / 独家 454 / **音擎 189 / 邦布 416**，共 1168 条；`my-account.js` 含四池保底与 S 级列表（2026-08 起该文件改为示例占位，真实数据走本地同步）
...
- 测试 65 项（+4 数据源格式测试）
- **真实账号解析**（2026-08-15，uid 已脱敏）：`src/data/my-account.js`，UI 支持「载入我的真实账号」一键套用 box / 保底 / 大保底（2026-08 起改为「账号同步」UID 流程）
